...
if(!empty($search_tag)){
  ...
  if($current_user->has_cap('manage_options')){
    $query = ("SELECT Task.*
              ...
              where Task.task_name LIKE '$search_tag_text' OR proj.project_name LIKE '$search_tag_text' OR task_statuses.name LIKE '$search_tag_text' OR task_priorities.name LIKE '$search_tag_text' OR user.display_name LIKE '$search_tag_text' Group by Task.id ORDER BY $sort_by $order ");
...
$no_of_rows = ("SELECT count(*) FROM 
  ...
  if($current_user->has_cap('manage_options')){
    ...
    if($sort_by=='task_name'|| $sort_by=='start_date' || $sort_by=='end_date'){
      $query = ( "SELECT * FROM {$wpdb->prefix}wppm_task ORDER BY $sort_by $order");
    } else{
...
$no_of_rows = apply_filters('wppm_task_list_no_of_rows',$no_of_rows);
                ...
                <tr onclick= "if(link)wppm_open_task(<?php echo esc_attr($task->id) ?>)" >
                  ...
                  <?php if($wppm_default_task_date == 1) { ?>
                          ...
                          <?php if(!empty($task->start_date)){ ?>
                                    <?php echo esc_html_e($task->start_date,'taskbuilder');
                                } ?>
                          </td>
                          <td>
                          <?php if(!empty($task->end_date)){ ?>
                                  <?php echo esc_html_e($task->end_date,'taskbuilder');
                                } ?>
                          </td>
                  <?php } ?>
                  </td>
