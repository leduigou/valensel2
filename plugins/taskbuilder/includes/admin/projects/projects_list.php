...
if(!empty($search_tag)){
  ...
  if($current_user->has_cap('manage_options')){
    $query = ("SELECT Project.*
              ...
              where Project.project_name LIKE '$search_tag_text' OR proj_statuses.name LIKE '$search_tag_text' OR proj_categories.name LIKE '$search_tag_text' OR ( user.display_name LIKE '$search_tag_text')
              Group by Project.id ORDER BY $sort_by $order");
    $no_of_rows = ("SELECT count(*) FROM 
              (SELECT Project.*
              FROM {$wpdb->prefix}wppm_project AS Project 
              Left join {$wpdb->prefix}wppm_project_statuses proj_statuses ON Project.status = proj_statuses.id
              Left join {$wpdb->prefix}wppm_project_categories proj_categories ON Project.cat_id = proj_categories.id
              Left join {$wpdb->base_prefix}users user ON (FIND_IN_SET(user.ID,Project.users)>0) AND (user.display_name LIKE '$search_tag_text')
              where Project.project_name LIKE '$search_tag_text' OR proj_statuses.name LIKE '$search_tag_text' OR proj_categories.name LIKE '$search_tag_text' OR ( user.display_name LIKE '$search_tag_text')  Group by Project.id) AS Project" );
  }else{
    $query = ("SELECT Project.*
            FROM {$wpdb->prefix}wppm_project AS Project 
            Left join {$wpdb->prefix}wppm_project_statuses proj_statuses ON Project.status = proj_statuses.id
            ...
            where (Project.project_name LIKE '$search_tag_text' OR proj_statuses.name LIKE '$search_tag_text' OR proj_categories.name LIKE '$search_tag_text' OR ( user.display_name LIKE '$search_tag_text')) AND ((FIND_IN_SET('$current_user->ID',Project.users)>0) OR (Project.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1))
            Group by Project.id ORDER BY $sort_by $order");
    $no_of_rows = ("SELECT count(*) FROM 
            (SELECT Project.*
            FROM {$wpdb->prefix}wppm_project AS Project 
  ...
  if($current_user->has_cap('manage_options')){
    if($sort_by=='project_name'|| $sort_by=='start_date' || $sort_by=='end_date'){
      $query = ( "SELECT * FROM {$wpdb->prefix}wppm_project ORDER BY $sort_by $order" );
      $no_of_rows = ( "SELECT count(*) FROM {$wpdb->prefix}wppm_project" );
    } else{
      ...
      $no_of_rows = ( "SELECT count(*) FROM (SELECT Project.* FROM {$wpdb->prefix}wppm_project AS Project
      Left join {$wpdb->prefix}wppm_project_statuses proj_statuses ON Project.status = proj_statuses.id
      Left join {$wpdb->prefix}wppm_project_categories proj_categories ON Project.cat_id = proj_categories.id
      Left join {$wpdb->prefix}wppm_project_meta proj_meta ON Project.id = proj_meta.project_id ) AS Project"); 
    }
  }else{
    if($sort_by=='project_name'|| $sort_by=='start_date' || $sort_by=='end_date'){
        $query = ( "SELECT Project.* FROM {$wpdb->prefix}wppm_project AS Project
          Left join {$wpdb->prefix}wppm_project_meta proj_meta ON Project.id = proj_meta.project_id
          where ((FIND_IN_SET('$current_user->ID',Project.users)>0 OR (Project.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1))) Group by Project.id ORDER BY $sort_by $order ");
        $no_of_rows = ( "SELECT count(*) FROM (SELECT Project.* FROM {$wpdb->prefix}wppm_project AS Project
          Left join {$wpdb->prefix}wppm_project_meta proj_meta ON Project.id = proj_meta.project_id
          where ((FIND_IN_SET('$current_user->ID',Project.users)>0 OR (Project.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1))) Group by Project.id) AS Project");
    } else{
      $query = ( "SELECT Project.* FROM {$wpdb->prefix}wppm_project AS Project
        Left join {$wpdb->prefix}wppm_project_statuses proj_statuses ON Project.status = proj_statuses.id
        Left join {$wpdb->prefix}wppm_project_categories proj_categories ON Project.cat_id = proj_categories.id
        Left join {$wpdb->prefix}wppm_project_meta proj_meta ON Project.id = proj_meta.project_id
        where ((FIND_IN_SET('$current_user->ID',Project.users)>0 OR (Project.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1))) ORDER BY $sort_by $order ");
      $no_of_rows = ( "SELECT count(*) FROM (SELECT Project.* FROM {$wpdb->prefix}wppm_project AS Project
        Left join {$wpdb->prefix}wppm_project_statuses proj_statuses ON Project.status = proj_statuses.id
        Left join {$wpdb->prefix}wppm_project_categories proj_categories ON Project.cat_id = proj_categories.id 
        Left join {$wpdb->prefix}wppm_project_meta proj_meta ON Project.id = proj_meta.project_id
        where ((FIND_IN_SET('$current_user->ID',Project.users)>0 OR (Project.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1)))) AS Project");
    }
  }
}
$no_of_rows = apply_filters('wppm_project_list_no_of_rows',$no_of_rows);
            ...
            foreach( $wppm_fillter as $project ){ 
                ...
                <tr onclick="wppm_open_project(<?php echo esc_attr($project->id) ?>)">
                  ...
                  <?php if($wppm_default_project_date==1){ ?>
                          ...
                          <td class="wppm_table_td"><?php echo isset($proj_end_date) ? esc_html($proj_end_date):"" ?></td>
                  <?php } ?>
