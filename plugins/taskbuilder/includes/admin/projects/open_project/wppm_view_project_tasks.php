...
if(!empty($search_tag)){
	...
	if($current_user->has_cap('manage_options')){
		$query = ("SELECT Task.*
					...
					where (Task.task_name LIKE '$search_tag_text' OR proj.project_name LIKE '$search_tag_text' OR task_statuses.name LIKE '$search_tag_text' OR task_priorities.name LIKE '$search_tag_text' OR user.display_name LIKE '$search_tag_text') Group by Task.id");
		$no_of_rows = ("SELECT count(*) FROM 
					...
					Left join {$wpdb->prefix}wppm_task_priorities task_priorities ON Task.priority = task_priorities.id
					Left join {$wpdb->base_prefix}users user ON (FIND_IN_SET(user.id,Task.users)>0) AND user.display_name LIKE '$search_tag_text'
					where (Task.task_name LIKE '$search_tag_text' OR proj.project_name LIKE '$search_tag_text' OR task_statuses.name LIKE '$search_tag_text' OR task_priorities.name LIKE '$search_tag_text' OR user.display_name LIKE '$search_tag_text') Group by Task.id) AS Task");
	} else{
		$query = ("SELECT Task.*
              FROM {$wpdb->prefix}wppm_task AS Task
              Left join {$wpdb->prefix}wppm_project proj ON Task.project = proj.id
	...
	if($current_user->has_cap('manage_options')){
		$query = ( "SELECT * FROM {$wpdb->prefix}wppm_task");
		$no_of_rows = ( "SELECT count(*) FROM {$wpdb->prefix}wppm_task" );
	}else{
		$query = ( "SELECT Task.*
			FROM {$wpdb->prefix}wppm_task AS Task
			Left join {$wpdb->prefix}wppm_project proj ON Task.project = proj.id
...
<form name="wppm_view_project_task" id="wppm_view_project_task">
									...
									<div class="wppm_card_body" onclick="wppm_open_task(<?php echo esc_attr($task->id)?>)" id="wppm_draggable_card_<?php echo esc_attr($task->id)?>">
										...
										<div class="row">
											<div class="col-sm-6 wppm_task_due_date_grid_view">
												<small><?php echo (isset($task->end_date))? esc_html_e($task->end_date,'taskbuilder') :"" ?></small>
											</div> 
	...
	<div class="row wppm_task_pagination_container">
		<div class="col-sm-4">
			<?php echo esc_html_e('Total:','taskbuilder'); ?>&nbsp;<?php echo esc_html($total_tasks); ?>&nbsp;<?php echo esc_html_e( 'of','taskbuilder') ?>&nbsp;<?php echo esc_html($totalrows) ?>&nbsp;<?php echo esc_html_e('Tasks','taskbuilder')?>
		</div>
