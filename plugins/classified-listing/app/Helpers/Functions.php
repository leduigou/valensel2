...
class Functions {
	...
	public static function is_billing_address_disabled() {
		return (bool) apply_filters( 'rtcl_billing_address_disabled',
			Functions::get_option_item( 'rtcl_payment_settings', 'billing_address_disabled', false, 'checkbox' ) );
	}
	...
	public static function format_price_range( $from, $to, $args = [] ) {
		...
		$price = sprintf( _x( '<div class="rtcl-price-range">%1$s <span class="sep">&ndash;</span> %2$s</div>', 'Price range: from-to', 'classified-listing' ),
			is_numeric( $from ) ? self::price( $from, false, $args ) : $from, is_numeric( $to ) ? self::price( $to, false, $args ) : $to );
	...
	public static function price( $price, $only_formatted_price = false, $args = [] ) {
		 ...
		 * @param float|string $original_price     Original price as float, or empty string. Since 5.0.0.
		 */
		$price = apply_filters( 'formatted_rtcl_price', number_format( $price, $args['decimals'], $args['decimal_separator'], $args['thousand_separator'] ),
			$price, $args['decimals'], $args['decimal_separator'], $args['thousand_separator'], $original_price, $args );
		...
		$formatted_price = ( $negative ? '-' : '' ) . sprintf( $args['price_format'], '<span class="rtcl-price-currencySymbol">' . $currency_symbol . '</span>',
				$price );
	...
	public static function get_payment_formatted_price( $price ) {
		 ...
		 * @param float|string $original_price     Original price as float, or empty string. Since 5.0.0.
		 */
		$price = apply_filters( 'formatted_rtcl_payment_price', number_format( $price, $decimals, $decimal_sep, $thousands_sep ), $price, $decimals,
			$decimal_sep, $thousands_sep, $original_price );
	...
	public static function get_payment_formatted_price_html( $price ) {
		...
		$payment_price_meta_html      = $payment_price_meta_html ? apply_filters( 'rtcl_payment_price_meta_wrap_html',
			sprintf( '<span class="rtcl-payment-price-meta">%s</span>', $payment_price_meta_html ), $payment_price_meta_html ) : null;
		...
		return apply_filters( 'rtcl_get_payment_formatted_price_html', $payment_price_html, $formatted_price, $original_price, $currency_symbol,
			$price_format );
	}
			...
			foreach ( $terms as $term ) {
				...
				if ( $term->taxonomy === rtcl()->category ) {
					...
					if ( $image_id = get_term_meta( $term->term_id, '_rtcl_image', true ) ) {
						...
						if ( '' !== $image ) {
							$termItem['icon'] = sprintf( '<span class="icon"><img src="%s" alt="%s" class="rtcl-cat-img" /></span>', esc_url( $image ),
								esc_attr( $term->name ) );
						}
					...
					if ( 'multiple_date_ranges' === $date_type ) {
						...
						if ( ! empty( $date_range_count ) ) {
							...
							for ( $i = 0; $i < $date_range_count; $i ++ ) {
								...
								$value           = $end ? date( $date_format, strtotime( $start ) ) . " - " . date( $date_format, strtotime( $end ) )
									: date( $date_format, strtotime( $start ) );
								$field_html      .= sprintf(
	...
	public static function get_listing_images( $post_id ) {
		...
		if ( function_exists( 'rtcl_fn_get_listing_child_raw_images' ) ) {
			...
			global $wpdb;
			$children
				= $wpdb->get_results( "SELECT ID FROM {$wpdb->posts} WHERE post_type = 'attachment' AND post_status = 'inherit' AND post_parent = {$post_id}",
				OBJECT_K );
		}
