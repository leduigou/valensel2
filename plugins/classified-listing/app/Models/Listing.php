...
class Listing extends Data
{
	...
	function get_the_thumbnail($size = 'rtcl-thumbnail')
	{
		...
		if ($thumb_id) {
			$image = wp_get_attachment_image($thumb_id, $size, false, [
				...
				"alt"   => esc_attr($this->get_the_title())
			]);
		} else {
	...
	public function the_gallery()
	{
		if (!Functions::is_gallery_disabled()) {
			...
			Functions::get_template("listing/gallery", ['images' => $this->get_images(), 'videos' => $video_urls, 'listing' => $this]);
		}
	}
		...
		if ($category) {
			...
			$query_args['tax_query'] = [
				[
					...
					'field'            => 'term_id',
					'terms'            => $category,
					'include_children' => isset($this->general_settings['include_results_from']) && in_array(
						'child_categories',
